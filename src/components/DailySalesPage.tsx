...
export function DailySalesPage() {
  const [activeTab, setActiveTab] = useState<DailySalesTabId>("dashboard");
    ...
    <main className="mx-auto max-w-7xl p-6">
      <Card className="gap-0 border-slate-200 shadow-sm">
        <CardHeader className="pb-0">
          <CardTitle className="text-2xl font-semibold text-slate-900">Daily Sales</CardTitle>
        </CardHeader>
        <CardContent className="pt-4">
          <DailySalesTabs activeTab={activeTab} onTabChange={setActiveTab} />
        </CardContent>
      </Card>

      {activeTab === "dashboard" ? <DashboardTab refreshTick={refreshTick} /> : null}
      {activeTab === "encoder" ? <EncoderTab onSaved={triggerRefresh} /> : null}
      {activeTab === "reports" ? <ReportsTab refreshTick={refreshTick} onChanged={triggerRefresh} /> : null}
      {activeTab === "inventory-report" ? <InventoryReportTab refreshTick={refreshTick} /> : null}
      {activeTab === "sales-report" ? <SalesReportTab refreshTick={refreshTick} /> : null}
      {activeTab === "users" ? <UsersTab /> : null}
      {activeTab === "sales-metrics" ? <SalesMetricsTab refreshTick={refreshTick} /> : null}
    </main>
  );
}
